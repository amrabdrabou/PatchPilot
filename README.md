...
- Archives `/clear` conversations into local JSON-backed history and lets saved conversations be loaded or deleted from the left panel.
...
- Saved conversations are written to `data/conversations.json`, which is ignored by git and can be deleted from the UI.
...
13. Done: Add a saved-conversation delete control in the frontend.
